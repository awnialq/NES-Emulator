...
p_bench = [x[63:].split()[0][2:] for x in lines_bench]      # Extract status register (P:)
...
p_exp = [x.split()[9] for x in lines]                       # Extract status register (p:)
...
for i in range(min(len(progc_bench), len(progc_exp))):
    if (progc_bench[i].lower() != progc_exp[i].lower() or
        ...
        p_bench[i].lower() != p_exp[i].lower() or
        stackp_bench[i].lower() != stackp_exp[i].lower()):
        print(f"Mismatch at line {i+1}:")
        print(f"  Benchmark: pc={progc_bench[i]} a={accum_bench[i]} x={x_bench[i]} y={y_bench[i]} p={p_bench[i]} sp={stackp_bench[i]}")
        print(f"  Emulator : pc={progc_exp[i]} a={accum_exp[i]} x={x_exp[i]} y={y_exp[i]} p={p_exp[i]} sp={stackp_exp[i]}")
        break
else:
